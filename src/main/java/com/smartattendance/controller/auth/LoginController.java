...
public class LoginController {
    ...
    private void handleLogin() {
        ...
        appLogger.info("Running Login For the User");
        ...
        try {
            ...
            appLogger.info("User Authenticated Successfully: " + user.getId());
            ...
            if (role == Role.STUDENT) {
                ...
                appLogger.info("Student face data status: " + (hasFaceData ? "enrolled" : "not enrolled"));

                if (!hasFaceData) {
                    // Student has no face data - redirect to enrollment
                    appLogger.info("Redirecting student to enrollment: " + user.getUserName());
            ...
            appLogger.info("User Authorized Successfully: " + user.getRole().name());

        } catch (Exception e) {
            String errorMsg = e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName();
            errorLabel.setText("Login failed: " + errorMsg);
            appLogger.error("Error during login portal loading for user: " + usernameField.getText(), e);
        }
    }
    ...
    private void handleForgetPassword() {
        try {
            ...
            // Logger to let dev know which function is running
            appLogger.info("Running Forgot Password For the User");
            ...
            appLogger.info("User Auth Verification Type: " + verificationType.name());
            ...
            appLogger.info("Verification Queue Inserted Successfully");
            ...
            appLogger.info("User Password Status: " + hasPassword);
            ...
            if (hasPassword) {
                ...
                appLogger.info("User redirected to password reset screen.");

            } else {
                // User has no password - show registration view
                loadRegistrationPortal(user);
                appLogger.info("User redirected to registration screen. Enrollment will happen on next login.");
            }
