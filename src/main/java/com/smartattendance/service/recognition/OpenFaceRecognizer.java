...
public class OpenFaceRecognizer extends Recognizer {
     ...
     * The model file (nn4.small2.v1.t7) must be present in src/main/resources/openFaceModels/
     */
     ...
     */
    private double euclideanDistance(Mat embedding1, Mat embedding2) {
        if (embedding1.empty() || embedding2.empty()) {
        ...
        Mat diff = new Mat();
        org.opencv.core.Core.subtract(embedding1, embedding2, diff);
            ...
            try {
                ...
                for (Mat rawImage : faceData.getImages()) {
                    ...
                    Mat preprocessedFace = faceProcessingService.preprocessFace(
                            ...
                            INPUT_WIDTH,
                            INPUT_HEIGHT,
                            false); // false = don't apply histogram equalization, keep RGB

                    if (preprocessedFace.empty()) {
                        continue;
                    }

                    // Ensure RGB format (OpenFace expects 3-channel RGB)
                    Mat rgbFace = new Mat();
                    if (preprocessedFace.channels() == 1) {
                        Imgproc.cvtColor(preprocessedFace, rgbFace, Imgproc.COLOR_GRAY2RGB);
                        preprocessedFace.release();
                    } else {
                        rgbFace = preprocessedFace;
                    }

                    // Compute embedding
                    Mat embedding = computeEmbedding(rgbFace);
                    rgbFace.release();
                ...
                // Note: Reusing histogram field - consider adding dedicated embedding field
                faceData.setHistogram(avgEmbedding);
    ...
    public RecognitionResult recognize(Mat faceImage, List<Student> enrolledStudents) {
        ...
        try {
            ...
            Mat inputEmbedding = computeEmbedding(rgbFace);
            rgbFace.release();
            ...
            for (Student student : enrolledStudents) {
                ...
                Mat storedEmbedding = faceData.getHistogram(); // Using histogram field for embeddings
                ...
                try {
                    ...
                    // Alternative: use Euclidean distance (uncomment if preferred)
                    // double distance = euclideanDistance(inputEmbedding, storedEmbedding);
                    // double similarity = 1.0 / (1.0 + distance); // Convert to similarity score
